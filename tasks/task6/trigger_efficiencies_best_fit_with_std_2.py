...
# Note:
# This file follows the second method of calculating the standard errors
# shift the values of efficiencies by a normal distribution
# with width of 1 sigma
# 
def main():
    ...
    # with the new values of Afb

    # 1 sigma up 
    np.random.seed(42)
    mup_eff_sigma_up = mup_eff + np.random.normal(0, 1, mup_eff_err.shape[0]) * mup_eff_err
    mum_eff_sigma_up = mum_eff + np.random.normal(0, 1, mum_eff_err.shape[0]) * mum_eff_err
    ...
    minima_std = (minima_sigma_up - minima)
    print(f"{minima_std=}")
    ...
    plt.legend()
    plt.savefig(f"{StoragePaths.plots_path}/best_fit_parabola_corrected_eta_trigger_eff_with_std_2.png")
