...
def main():
    ...
    wma = [0.240, 0.250, 0.260, 0.280, 0.310]
  ...
  # err_interpolation = 
    ...
    for i, sin2theta in enumerate(wma):
        plt.scatter(m_ll, interpolation_values[i], label=f"Interpolated $sin2theta_w$={wma[i]:.3}")
    #plt.errorbar(m_ll,interpolation_values[0],   )
    
    plt.legend()
    plt.savefig(f"{StoragePaths.plots_path}/template_and_linear_interpolations.png")
